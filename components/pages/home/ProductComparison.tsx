...
const ProductComparison = () => {
  ...
  // Close first search results when clicking outside
  useEffect(() => {
  ...
  // Close second search results when clicking outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
  ...
  const handleSelectedProductOne = (product: Product) => {
    setProductOne(product);
    setSearchOne(product?.name  as string);
    setShowResultsOne(false);
  };

  const handleSelectedProductTwo = (product: Product) => {
  ...
  return (
    <div className="bg-yellow-100 rounded-md p-4 flex flex-col justify-center items-center h-1/2">
      ...
      <div className="space-y-2 w-full mt-2">
              ...
              {loadingOne ? (
                ...
                <div>
                  {productsOne.map((product) => (
                    <div
                      key={product?._id}
                      onClick={() => handleSelectedProductOne(product)}
              ...
              {loadingTwo ? (
                ...
                <div>
                  {productsTwo.map((product) => (
                    <div
                      key={product?._id}
                      onClick={() => handleSelectedProductTwo(product)}
                      className="px-3 py-3 hover:bg-gray-100 cursor-pointer border-b border-gray-100 last:border-b-0"
                    >
                      <p className="text-sm font-medium line-clamp-1">
                        {product?.name}
        ...
        {/* Compare Button */}
        <button onClick={handleComparison} disabled={!productOne || !productTwo} className={`border rounded-sm w-full text-sm font-semibold py-1.5 transition-colors 
        ${productOne && productTwo ? "bg-transparent border-tech_blue hover:bg-tech_blue hover:text-tech_white " : "bg-gray-100 border-gray-300 text-gray-400 cursor-not-allowed"}` } 
        >
          View Comparison
        </button>
      </div>
